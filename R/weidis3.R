weidis3 <- function(x,m=1) {
  ...
  zz <- y[yy,]
   ...
   if(np!=0) {
    ...
    v1b <- ell1 +v2
    n2 <- length(xp2)
    ad1 <- matrix(0,nrow=(r1+1),ncol=v1b)
    for(i in 1:(r1+1)) {
      j1 <- (i-1)*m + 1
      j2 <- j1 + (n2-1)
      ad1[i,j1:j2] <- coef(xp2)
    }
    ad2 <- matrix(0,nrow=(r1+1),ncol=ell1)
    ad2[,1:ell1] <- ad1[,(v2+1):v1b]
    n3 <- (n2+1)/2
    for(i in 2:n3) {
      ad2[1,i] <- 2* ad2[1,i]
    }
...
nin1 <- max(ng)-min(ng)
  ...
  if(od1[1]!=0) {
    ...
    if(od1[1]==2) {
      xv.vec <- c(xv,xv[nv2]*coef(gp3)[2]^(1:(nw-nv1-d1)) + xv[(nv2-1)]*coef(gp3)[1]^(1:(nw-nv1-d1)))
    }
